...
const pluginPickerStyles = {
    ...
    multiValueLabel: (styles, {data}) => {
        if(!data.__isNew__)
            return {...styles, color: "rgb(0, 82, 204)"};
        else
...
function PluginPicker({onChange}) {
    ...
    const onSelectChange = useCallback((currentValue) => {
    ...
    return (
        <div>
            <CreatableSelect isMulti options={plugins} styles={pluginPickerStyles} onChange={onSelectChange} defaultValue={defaultPlugins}/>
            {
                chosenCustomPlugin ?
...
export default function UploadView(props) {
  return (
      <div className="container-fluid px-4">
          ...
          <div className="mb-3">
              <label htmlFor="formFile" className="form-label">Sample file</label>
              <input className="form-control" type="file" id="formFile"/>
          </div>
          <div className="mb-3">
              <label htmlFor="customRange2" className="form-label">Analysis time</label>
              <input type="range" className="form-range" min="0" max="15" id="customRange2"/>
          </div>
          <div className="mb-3">
              <label htmlFor="customRange2" className="form-label">Plugins</label>
              <PluginPicker/>
          </div>
          <div className="mb-3">
              <label htmlFor="exampleFormControlInput1" className="form-label">Custom file name</label>
              <input type="text" className="form-control" id="exampleFormControlInput1"/>
          </div>
          <div className="mb-3">
              <label htmlFor="exampleFormControlInput1" className="form-label">Start command</label>
              <input type="text" className="form-control" id="exampleFormControlInput1"/>
          </div>
      </div>
  )
